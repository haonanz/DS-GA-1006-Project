...
if __name__ == '__main__':
    ...
    for depth in max_depths:
        ...
        print eval_results
        train_results = [float(x[0]) for x in eval_results]  # Note MSE prints
        val_results = [float(x[1]) for x in eval_results]  # Note MSE prints
    ...
    model_output_dir = os.path.join(os.path.dirname(os.path.realpath(__file__)), 'model_output/xgb2')
    depth_all_results.to_csv(os.path.join(model_output_dir, 'XGB2_depth_learning_curve.csv'), index=False)
    colors = ['b', 'g', 'r', 'k']
    ...
    fig.savefig(os.path.join(model_output_dir, 'XGB2_depth_learning_lc.png'))
    ...
    all_results.to_csv(os.path.join(model_output_dir, 'XGB2_learning_curve.csv'), index=False)
    colors = ['b', 'g', 'r', 'k']
    ...
    plt.ylabel('Root Mean Squared Percentage Error')
    plt.xlabel('Number of Rounds of Boosting')
    fig.savefig(os.path.join(model_output_dir, 'XGB2_num_round_eta_lc.png'))
    ...
    plt.ylabel('Feature')
    plt.tight_layout()
    plt.savefig(os.path.join(model_output_dir, 'XGB2_feature_importance.png'))
    ...
    evaluate_model('XGB2', predicted_test, y_test, model_output_dir)

    print "Saving model..."

    bst_final.save_model(os.path.join(model_output_dir, 'XGB2_final_model.model'))
