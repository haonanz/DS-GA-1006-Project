...
if __name__ == '__main__':
    ...
    y_train = np.log10(df_train['price'])
    y_scaler = preprocessing.StandardScaler().fit(y_train)
    y_train =  y_scaler.transform(y_train)
    X_test = get_X_dataframe(df_test)
    y_test = np.log10(df_test['price'])
    ...
    depth_all_results.to_csv('./saved_models/XGB_model_3_depth_learning_curve.csv', index=False)
    colors = ['b','g','r','k']
    ...
    fig.savefig('./figs/XGB_model_3_depth_learning_lc.png')
    ...
    all_results.to_csv('./saved_models/XGB_model_3_learning_curve.csv', index=False)
    colors = ['b','g','r','k']
    ...
    plt.ylabel('Mean Squared Error')
    plt.xlabel('Number of Rounds of Boosting')
    fig.savefig('./figs/XGB_model_3_num_round_eta_lc.png')
    ...
    plt.ylabel('Feature')
    plt.tight_layout()
    plt.savefig('./figs/XGB_model_3_feature_importance.png')
    ...
    predicted_test = y_scaler.inverse_transform(predicted_test)
    predicted_test = np.power(predicted_test,10)
    y_test = y_scaler.inverse_transform(y_test)
    y_test = np.power(y_test,10)

    metrics = [mean_squared_error, r2_score, mean_absolute_error, median_absolute_error]
    with file('./saved_models/XGB_model_3_test_scores.txt','w') as test_output:
    ...
    plt.legend()
    plt.xlim(0,200)
    plt.savefig('./figs/XGB_model_3_perc_error_hist.png')

    pd.DataFrame(predicted_test, index=X_test.index).to_csv('./saved_models/XGB_model_3_test_predictions.csv', index=True)

    print 'Saving final xgb model...'
    bst_final.save_model('./saved_models/XGB_model_3_final_model.model')
